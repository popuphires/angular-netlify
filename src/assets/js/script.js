...
function cws_touch_events_fix() {
  if (is_mobile_device()) {
    jQuery(".container").on(
      "mouseenter",
      ".hover-effect, .product .pic",
      function(e) {
        e.preventDefault();
        jQuery(this).trigger("hover");
      }
    );
    jQuery(".features-tours-item").on(
      "mouseenter",
      ".features-info-top, .features-info-bot",
      function(e) {
        e.preventDefault();
        jQuery(this).trigger("hover");
      }
    ...
    jQuery(".main-nav").on(
      "hover",
      ".mobile_nav .button_open, .mobile_nav li > a",
      function(e) {
        e.preventDefault();
...
function search_open() {
  ...
  $(".site-top-panel .search_menu_cont .search_back_button").on(
    "click",
    function() {
...
function init_twitter_carusel() {
  if ($(".twitter-1").length) {
    $(".twitter-1").tweet({
      ...
      template:
        "<i class='fa fa-twitter twitt-icon'></i><p><a href='{user_url}'>@{screen_name}</a>{join}{text}<br>{time}</p>"
    });
  ...
  $(".twitter-footer").tweet({
    ...
    loading_text: "loading twitter feed...",
    template:
      "<i class='fa fa-twitter twitt'></i><p><a class='link-color' href='{user_url}'>@{screen_name}</a>{join}{text}<br>{time}</p>"
  });
}
...
function woocommerce_price_slider() {
  ...
  $("body").on("price_slider_create price_slider_slide", function(
    event,
    min,
    max
  ) {
      ...
      /*feedback-form*/ {
        ...
        submitHandler: function(form) {
          ...
          submit_handler(
            ...
            $(form)
              .parent()
              .children(".email_server_responce")
          );
        }
      }
    );
    ...
    $.post("php/contacts-process.php", data, function(s_response) {
      ...
      if (s_response.info == "success") {
        ...
        $wrapper
          ...
          .append(
            '<div role="alert" class="alert alert-warning alt alert-dismissible fade in"><button type="button" data-dismiss="alert" aria-label="Close" class="close"></button><i class="alert-icon flaticon-warning"></i><strong>Error!</strong><br>Server fail! Please try again later!</div>'
          );
...
function mobile_nav_switcher_init() {
  ...
  jQuery(document).on(
    "click",
    "header nav .inner-nav.mobile_nav .mobile_menu_switcher",
    function() {
...
function get_current_nav_level() {
  var r =
    window.mobile_nav.level < 2
      ? jQuery("header nav .inner-nav>ul")
      : jQuery("header nav .inner-nav ul");
  r.find("ul").remove();
  return r;
}
...
function reset_mobile_menu() {
  ...
  $("header nav .inner-nav")
    .removeClass("mobile_nav opened")
    .addClass("desktop-nav");
